...
def test_file_iteration_tell():
    ...
    for line in f.readline():
        tells.append(f.tell())
    print(tells)
    assert [4, 4, 4, 4] == tells # is there really no difference between readlines and iteration? # readlines returns a list of lines while iteration directly iterates over a file as an iterator
...
-Python has an extensive file handling API, which is very useful
-Reading and writing data onto files differs in binary and text modes and in different operating systems
-It is important to release files by using file.close()
"""