...
def test_modules_are_cached():
    ...
    try:
        inner()
    except AttributeError:  # what exception do you get here?     # AttributeError
        pass
...
def test_module_star_import():
    ...
    assert {"m3_func1", "m3_func2"} == (s2 - s1)  # what did module3 import bring in.   # two functions
    assert {"m4_func1", "_m4_func3"} == (s3 - s2)  # what did module4 import bring in.  # functions specified in __all__
...
Modules can be used to build large programs with data and functions in different files
Imports can be done anywhere, but * imports cannot be done within functions
Namespace with respect to modules
"""
