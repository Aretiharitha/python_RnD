...
def test_iterator_type():
    ...
    print(type(tuple_iter).__name__)
...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [] == result  # what happened here?   # the iterable iter1 already reached it's final value in the previous loop
...
def test_find_builtins_that_work_on_iterables():
    funcs = [all, any, filter, list, map, max, min, set, sorted, sum, tuple, zip]
    assert 12 == len(funcs)
...
def my_map(func, iterable):
    ...
    try:
        while True:
            result.append(func(iterable.__next__()))
    except StopIteration:
        return result
    pass
...
def test_my_map():
    ...
    #assert list(string.ascii_uppercase) == my_map(string.ascii_uppercase, string.ascii_lowercase)
...
Python has iterators to represent and traverse through streams of data
StopIteration is an exception used to stop the iteration when the iterator is exhausted
We can get iterator objects for sequences like str, list, tuple, set and dict
"""
