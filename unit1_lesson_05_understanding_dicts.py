...
def test_valid_dictionary_keys():
    ...
    try:
        key = (1, 2)
        test_dict[key] = "tuple with immutable elements1"
    except TypeError as te:
        print("See me in pytest output?", te, sep="\n")  # observe the error message
    ...
    try:
        ...
        assert True  # do we reach here?
    print(test_dict)
    assert {1: 1, 'one': 'string', (1, 2): 'tuple with immutable elements1'} == test_dict
...
def make_dict(keys, values):
    ...
    return d  # write a single line of code to create the dict.
