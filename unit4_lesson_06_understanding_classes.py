...
def test_classes_init_with_args():
    ...
    try:
        q3 = TestQueue()
    except TypeError: #what error do you get?      # TypeError, as __init__() takes exactly two arguments, but none is specified (i.e., one is implicitly specified which is self)
        pass
...
def test_instances_state():
    ...
    try:
        age = r1.age
        assert False, "Should not reach here"
    except AttributeError: #what exception do you get?  #AttributeError as Record object has no attribute 'age'
        assert True
...
We can create user-defined types in Python using classes
Classes can have their own state (data attributes) as well as behaviour (methods)
We can add attributes to an instance of a class at runtime
"""