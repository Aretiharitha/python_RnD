...
def get_multiples_desc(number, count):
    mul_list = [1*number,2*number,3*number,4*number,5*number,6*number]
...
def ascii_distance(first, second):
    return abs(ord(second) - ord(first))
    # use 2 builtin functions to get your work done.
...
def get_binary(number):
    ...
    # use a builtin and slice the result ...
