...
def test_function_call_with_keyword_arguments():
    assert [10, 0, 3, 4] == demo(10,0)
    assert [10, 20, 3, 4] == demo(10, 20)
    assert [10, 20, 30, 4] == demo(10, 20, 30)
    assert [0, 20, 3, 4] == demo(0 ,second=20)
    assert [0, 20, 30, 4] == demo(0, second=20, third=30)
    assert [10, 0, 30, 4] == demo(first=10, second=0, third=30)
    assert [10, 0, 30, 4] == demo(10, 0, third=30)
...
def simple_format(format, *args):
    ...
    for x in range(args.__len__()):
        format=format.replace(("%"+str(x)), args[x])
    return  format
